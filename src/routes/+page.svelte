    ...
    const columns:Array<DataColumn | CompositeColumn> = [
        {
            ...
            align: "left",
            sortable: true,
            href: item => {return {text: item.applicationNo, action: () => {console.log(item.id, item.applicationNo)}}}
        },
        {
            field: "arrivalDate",
            text: "入厂日期",
...
<div style="width: 800px; height: 600px; font-size: 12px; position: relative; margin: 10px auto; box-sizing: border-box;">
    <AdvanceDataTable {columns} list={list} {options}/>
</div>
